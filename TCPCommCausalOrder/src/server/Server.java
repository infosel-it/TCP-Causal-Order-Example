...
public class Server {
	...
	public <E extends Serializable> void sendMessageToRoom(Message<E> message, Room room) {
		if (room != null & message != null) {
			System.out.printf("%s -> %s(%d) [type = %s]: %s\n", message.getSender(), room.getName(), room.getId(),
					message.getType(), message.getContents());

			System.out.println(" Target Client Name :" + message.getTargetClientName());
			
			if (message.getTargetClientName().length() != 0) {
				for (int userId : room.getUsers()) {

					System.out.println(" User ID :" + userId);
					ClientHandler ch = this.clientConnections.get(userId);
					System.out.println("Channel Name : " + ch.clientName);
					
					if (message.getTargetClientName().equals(ch.clientName)) {
						if (ch != null) {
